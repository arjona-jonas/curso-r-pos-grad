...
1.
```{r}
str_subset(dados$nome_unidade_gestora,pattern="SÃO CARLOS") %>% 
  unique()
```

2.
```{r}
str_subset(dados$nome_unidade_gestora,pattern="UFSCAR") %>% 
  unique()
```
Há diferença, temos o Hospital Universitário (HU) e a Secretaria Geral de Informática (SIN).
...
dados %>% 
  ...
  arrange(desc(soma))
```
A maior soma total foi no dia 22, com 1.029.776,60 reais

4.
```{r}
str_subset(presidencia$nome_favorecido,pattern="LTDA") %>% 
  unique() %>% 
  head()
```
...
5.
```{r}
str_replace(dados$valor_transação,pattern="\\.",replacement = ",") %>% 
  head(n=10)
```

6.
```{r}
fabricio <- str_detect(dados$nome_portador,pattern="FABRICIO")

dados[fabricio,] %>% 
  arrange(desc(valor_transação))
```
O maior valor foi de 2.250 reais e foi feito pela Agência Nacional de Mineiração em RS, do Ministério de Minas e Energia.
